...
| **Execution Funnel** | **Done.** PRs #1287-#1291, #1375-#1379 (March 2026). Signal-to-P&L diagnostic pipeline: append-only CSV event log tracking each decision from council signal through order placement, fill, and realized P&L. Dashboard page (10_The_Funnel.py) strictly filters KPIs by actual filled trades and features an Outcome Breakdown matrix, Walk Cost analysis (adaptive limit walking), native-unit Signal P&L tracking, regime comparisons, and cycle drill-downs. Backfill script reconstructs funnel from historical `council_history.csv` + `trade_ledger.csv`. |
...
**Partial progress:** Contribution scoring (done) now tracks per-agent value-add vs the council decision. Execution Funnel (done) traces signal-to-P&L with walk cost analysis and outcome breakdowns. Master Strategist forensics (done) analyzes override patterns and agent agreement rates. Direction-evidence mismatch detection already flags agents whose directional call contradicts their own evidence. **Remaining:** formal NLI faithfulness checking, debate divergence metric, systematic mode-collapse detection.
