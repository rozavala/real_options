...
class TestStrategy(unittest.TestCase):
    ...
    def test_execute_bullish_strategy(self, mock_place_combo):
        async def run_test():
            ...
            await execute_directional_strategy(ib, config, signal, chain, 100.0, future_contract)
            ...
            self.assertEqual(legs_def[0], ('C', 'BUY', 100)) # ATM call
            self.assertEqual(legs_def[1], ('C', 'SELL', 102)) # OTM call
    ...
    def test_execute_bearish_strategy(self, mock_place_combo):
        async def run_test():
            ...
            self.assertEqual(len(legs_def), 2)
            self.assertEqual(legs_def[0], ('P', 'BUY', 100)) # ATM put
            self.assertEqual(legs_def[1], ('P', 'SELL', 98))  # OTM put
