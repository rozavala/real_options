...
5.  **State Management:** State reconciliation must group raw IBKR legs into semantic thesis groups, use aggregate quantities to avoid false-positive orphan detection, and accurately record all lots in multi-lot combo fills. Multi-commodity operations require strict isolation via `clientId` filtering (to prevent order cancellation across engines) and IBKR symbol prefix matching (e.g., `LN1` for Natural Gas weekly options) to prevent cross-engine order corruption and false untracked positions.
