...
10. **Observability:** An end-to-end Execution Funnel diagnostic pipeline tracks every decision from council signal through order placement, fill, and realized P&L. The system strictly isolates metrics to filled trades for accurate Outcome Breakdowns, tracking signal quality in native price units and monitoring adaptive limit "Walk Cost".
11. **Digest:** Post-close `System Health Digest` generation summarizes multi-commodity system state and errors for observability.
