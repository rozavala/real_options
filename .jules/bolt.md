...
## 2026-03-23 - Execution Funnel Observability & Metrics
**Learning:** When measuring end-to-end system performance (Execution Funnel), metrics must strictly filter by actual filled trades (`ORDER_FILLED`) rather than all council decisions. Using "Slippage" to describe adaptive order walking is misleading; "Walk Cost" is the correct term for distance traveled from initial limit to fill. Finally, "Signal P&L" measures the underlying price movement (signal quality) and should be displayed in native instrument units (e.g., `$/mmBtu` or `cents/lb`) rather than being converted to a dollar spread P&L, which introduces confusion given the nonlinear caps of options spreads.
**Action:** Always filter by `ORDER_FILLED` before computing KPI metrics. Use "Walk Cost" instead of "Slippage" for adaptive limit walking, and display "Signal P&L" in native units to accurately reflect prediction quality.
